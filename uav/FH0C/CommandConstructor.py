...
class CommandConstructorCore:
    ...
    def sendCommand(self, data: bytearray, max_retry=None, cmd_id_for_clean=None):
        """
        ...
        :param cmd_id_for_clean: Command ID for clean
        ...
        """
        self.q_write.put((QueueSignal.CMD, data, max_retry, cmd_id_for_clean), block=True)
        pass

    def cleanSendRetry(self, cmd_id_for_clean):
        self.q_write.put((QueueSignal.CLEAN, None, None, cmd_id_for_clean), block=True)
        pass
...
class CommandConstructor(CommandConstructorCore):
    ...
    def build_cmd_params(self, cmd: int, *param_bytes) -> tuple[bytearray, int]:
        """
    ...
    def _led(self, mode: int, r: int, g: int, b: int):
        ...
        (params, order_count) = self.build_cmd_params(13, mode, r, g, b)
    ...
    def takeoff(self, high: int, ):
        ...
        (params, order_count) = self.build_cmd_params(0, high_bytes, 50, 0)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
    ...
    def land(self, ):
        ...
        (params, order_count) = self.build_cmd_params(254, 0)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
        print("land", cmd.hex(' '))
        self.sendCommand(cmd)
    ...
    def move(self, direction: int, distance: int):
        ...
        (params, order_count) = self.build_cmd_params(5, direction, distance_bytes)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
        print("move", cmd.hex(' '))
        self.sendCommand(cmd)
    ...
    def flip(self, direction: int, circle: int):
        ...
        (params, order_count) = self.build_cmd_params(12, direction, circle)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
        print("flip", cmd.hex(' '))
        self.sendCommand(cmd)
    ...
    def arrive(self, x: int, y: int, z: int):
        ...
        y_bytes = pack("<h", y)
        z_bytes = pack("<h", z)
        (params, order_count) = self.build_cmd_params(9, x_bytes, y_bytes, z_bytes)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
    ...
    def rotate(self, degree: int):
        ...
        (params, order_count) = self.build_cmd_params(10, degree_bytes)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
    ...
    def speed(self, speed: int):
        ...
        (params, order_count) = self.build_cmd_params(2, speed_bytes)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
        print("speed", cmd.hex(' '))
        self.sendCommand(cmd)
    ...
    def high(self, high: int):
        ...
        (params, order_count) = self.build_cmd_params(11, high_bytes)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
        print("high", cmd.hex(' '))
        self.sendCommand(cmd)
    ...
    def airplane_mode(self, mode: int):
        ...
        (params, order_count) = self.build_cmd_params(1, mode)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
    ...
    def vision_mode(self, mode: int):
        ...
        (params, order_count) = self.build_cmd_params(0x0A, mode)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
    ...
    def hovering(self, ):
        ...
        (params, order_count) = self.build_cmd_params(254, 4)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
    ...
    def vision_color(self, L_L: int, L_H: int, A_L: int, A_H: int, B_L: int, B_H: int, ):
        ...
        *_L/*_H 为色彩在 Lab 色彩空间上各个的色彩通道的上下限范围
        ...
        (params, order_count) = self.build_cmd_params(0x0A, 0x06, L_L, L_H, A_L, A_H, B_L, B_H)
        cmd = self.join_cmd(CmdType.SINGLE_CONTROL, params)
