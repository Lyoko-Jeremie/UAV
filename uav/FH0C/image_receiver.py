...
class ImageReceiver:
    ...
    def _send_transfer_pack(self, pack_id: int):
        """
        构造并返回控制帧。
        pack_id=0x00000000: 请求从头开始发送（握手）
    ...
    def _on_packet_timeout(self):
        """
        超时回调：若总超时则强制结束；否则仅重置定时器，继续等待远端自动循环补包。
        """
        with self._lock:
            if self.image_instance is None:
                return
    ...
    def on_receive_image_packet_data(
        ...
        """
        收到数据帧（0xAA...0x0B...）。
        写入缓存，若收到最后序号包则输出 log，全部收齐后组装图片。
        不主动请求重传，依靠远端自动循环补包。
        """
        with self._lock:
            ...
            if self.user_progress_callback is not None:
                self.user_progress_callback(received, total)
    ...
    def on_receive_image_packet_data_eof(self, size_len: int, origin_data: bytearray):
        ...
        输出 log，检查是否全部收完；若未收完则仅等待远端下一轮自动补包，不主动重传。
        """
        with self._lock:
            if self.image_instance is None or self.image_instance.total_packets == 0:
                return

            self._has_sent_start = True  # EOF 到达说明无人机已在发送
            self.image_instance.last_packet_time = time.time()
            ...
            print(f"[image_transfer] EOF received: received={received}/{total}, "
                  f"first_lost={lost_mark if lost_mark < total else 'none'}.")
